...
export default async () => {
        ...
        choices: [
            {
                name: "Empty",
                value: "empty",
                description: "Start from an empty project, you can configure your own tools.",
            },
            {
                name: "Default Theme",
                value: "default",
    ...
    try {
        ...
        const theme = await fsClient.createTheme(config.name);
        if (template === "default") {
            const spinner = ora(`Cloning template`).start();
            execSync(`git clone --depth=1 --branch=main git@github.com:forumspark/default-theme.git ${config.name} && rm -rf ./${config.name}/.git`);
